...
public class DecalBuilder2 {
	...
	public static void BuildDecalForObject(Decal decal, GameObject affectedObject) {
		...
		for(int i=0; i<triangles.Length; i+=3) {
			...
			DecalPolygon2 poly = new DecalPolygon2( v1, v2, v3 );

			poly = DecalPolygon2.ClipPolygon(poly, right);
			if(poly == null) continue;
			poly = DecalPolygon2.ClipPolygon(poly, left);
			if(poly == null) continue;

			poly = DecalPolygon2.ClipPolygon(poly, top);
			if(poly == null) continue;
			poly = DecalPolygon2.ClipPolygon(poly, bottom);
			if(poly == null) continue;

			poly = DecalPolygon2.ClipPolygon(poly, front);
			if(poly == null) continue;
			poly = DecalPolygon2.ClipPolygon(poly, back);
			if(poly == null) continue;
	...
	private static void AddPolygon(DecalPolygon2 poly, Vector3 normal) {
